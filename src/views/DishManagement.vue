<template>
  <div class="dish-management">
    ...
    <div v-if="selectedItems.length > 0" class="bulk-actions">
      <div class="bulk-info">
        <span class="bulk-text">已选择 {{ selectedItems.length }} 个菜品</span>
...
const fetchDishes = async () => {
  ...
  try {
    ...
    toast.error('获取菜品列表失败: ' + error.message)
  } finally {
    loading.value = false
  }
}
...
const editDish = async dish => {
  try {
    ...
    toast.error('获取菜品详情失败: ' + error.message)
  }
}
...
const toggleDishStatus = async dish => {
  try {
    ...
    if (response.code === 0) {
      ...
      toast.success(`菜品已${newStatus ? '启用' : '禁用'}`)
      updateStats()
    } else {
...
const deleteDish = async dish => {
  if (!confirm(`确定要删除菜品"${dish.name}"吗？此操作不可恢复。`)) {
    return
  }
...
const bulkToggleStatus = async isActive => {
  ...
  if (!confirm(`确定要${action}选中的 ${selectedItems.value.length} 个菜品吗？`)) {
    return
  }
  ...
  try {
    ...
    if (successCount > 0) {
      toast.success(`成功${action} ${successCount} 个菜品`)
      clearSelection()
      fetchDishes()
    } else {
...
const bulkDelete = async () => {
  ...
  if (!confirm(`确定要删除选中的 ${selectedItems.value.length} 个菜品吗？此操作不可恢复。`)) {
    return
  }
  ...
  try {
    ...
    if (successCount > 0) {
      toast.success(`成功删除 ${successCount} 个菜品`)
      clearSelection()
      fetchDishes()
    } else {
